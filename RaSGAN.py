...
# Reference: https://github.com/AlexiaJM/RelativisticGAN/tree/master
...
parser.add_argument('--batch_size', type=int, default=8) # DCGAN paper original value used 128 (32 is generally better to prevent vanishing gradients with SGAN and LSGAN, not important with relativistic GANs)
parser.add_argument('--n_colors', type=int, default=3)
parser.add_argument('--z_size', type=int, default=128)
parser.add_argument('--dims', type=int, default=2048
parser.add_argument('--path', default=["./Extra/1", "./Train/Live"])
...
parser.add_argument('--input_folder', default='./Train/', help='input folder')
parser.add_argument('--output_folder', default='./GANlosses', help='output folder')
...
parser.add_argument('--load', default='', help='Full path to network state to load (ex: /home/output_folder/run-5/models/state_11.pth)')
parser.add_argument('--cuda', type='bool', default=True, help='enables cuda')
parser.add_argument('--n_gpu', type=int, default=1, help='number of GPUs to use')
parser.add_argument('--device', type=str, default=None, help='number of GPUs to use')
parser.add_argument('--loss_D', type=int, default=6, help='Loss of D, see code for details (1=GAN, 2=LSGAN, 3=WGAN-GP, 4=HingeGAN, 5=RSGAN, 6=RaSGAN, 7=RaLSGAN, 8=RaHingeGAN)')
parser.add_argument('--Diters', type=int, default=1, help='Number of iterations of D')
parser.add_argument('--Giters', type=int, default=2, help='Number of iterations of G.')
...
parser.add_argument('--gen_extra_images', type=int, default=100, help='Generate additional images with random fake cats in calculating FID (Recommended to use the same amount as the size of the dataset; for CIFAR-10 we use 50k, but most people use 10k) It must be a multiple of 100.')
parser.add_argument('--gen_every', type=int, default=1000, help='Generate additional images with random fake cats every x iterations. Used in calculating FID.')
...
parser.add_argument('--show_graph', type='bool', default=True, help='If True, show gradients graph. Really neat for debugging.')
...
parser.add_argument('--print_every', type=int, default=10, help='Generate a mini-batch of images at every x iterations (to see how the training progress, you can do it often).')
...
parser.add_argument('--CIFAR10_input_folder', default='/research/iprobe-tmp/yadavshi/PA_Detection/GANs/Datasets/CIFAR10', help='input folder (automatically downloaded)')
parser.add_argument('--metrics',      help='Quality metrics', default='fid50k_full')
...
curr_fid = 1000
delta = 0.05
while (i >= iter_offset and i < param.n_iter) or ((i+1) % param.gen_every == 0 and curr_fid - prev_fid > delta):
	...
	for t in range(param.Diters):
		...
		if param.loss_D in [1,2,3,4]:
			...
			feat, y_pred_fake = D(x_fake.detach())
			if param.loss_D == 1:
			...
			y.data.resize_(current_batch_size).fill_(0)
			y2.data.resize_(current_batch_size).fill_(1)
			z.data.resize_(current_batch_size, param.z_size, 1, 1).normal_(0, 1)
			fake = G(z)
			x_fake.data.resize_(fake.data.size()).copy_(fake.data)
			feat, y_pred_fake = D(x_fake.detach())
		...
		if (param.loss_D in [3] or param.grad_penalty):
			...
			outputs=D(x_both)
			#pdb.set_trace()
			grad = torch.autograd.grad(outputs=outputs[0].flatten(), inputs=x_both, grad_outputs=grad_outputs, retain_graph=True, create_graph=True, only_inputs=True)[0]
	...
	'''
	if(best_G > errG.data.item() and i >= 1000):
		best_G = errG.data.item()
		sts = 'Best G so far is: [%.4f]'
		ss = sts % (errG.data.item())
		torch.save({
				'i': i + 1,
				...
				'z_test': z_test,
			}, '%s/best_model/state_%05d.pth' % (param.extra_folder, i))
		print(ss)
	'''
	...
	if (i+1) % param.gen_every == 0:
		...
		if param.save:
			...
			torch.save({
				...
				'G_loss': errG.data.item(),
				'z_test': z_test,
			}, '%s/models/state_final.pth' % (param.extra_folder))
		...
		ext_curr = 0
		z_extra = torch.FloatTensor(10, param.z_size, 1, 1)
		if param.cuda:
			z_extra = z_extra.cuda()
		for ext in range(int(param.gen_extra_images/10)):
		...
		curr_fid = calculate_fid_given_paths(param.path,
                                          param.batch_size,
                                          param.device,
                                          param.dims,
                                          num_workers)
		
		if(curr_fid - prev_fid < delta):
			break
